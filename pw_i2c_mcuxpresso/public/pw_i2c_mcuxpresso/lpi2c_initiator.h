...
// Initiator interface implementation based on LPI2C driver in NXP MCUXpresso
// SDK. Currently supports only devices with 7 bit addresses.
class McuxpressoLpI2cInitiator final : public Initiator {
 public:
  struct Config {
  ...
  McuxpressoLpI2cInitiator(const Config& config)
      : Initiator(Initiator::Feature::kStandard),
        config_(config),
        base_(reinterpret_cast<LPI2C_Type*>(config_.flexcomm_address)) {}
  ...
  ~McuxpressoLpI2cInitiator() final;
 ...
 private:
  ...
  static void TransferCompleteCallback(LPI2C_Type* base,
  ...
  // inclusive-language: disable
  lpi2c_master_handle_t handle_ PW_GUARDED_BY(mutex_);
  // inclusive-language: enable
};
