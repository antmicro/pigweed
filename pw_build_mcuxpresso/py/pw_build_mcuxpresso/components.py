...
def parse_include_paths(
    ...
    Returns:
        dict of include directories for the component, key is condition for this item, or "None" if there isn't condition.
    """
    include_paths: dict[str, list[pathlib.Path]] = dict()
    include_paths["None"] = list()
    for include_type in ("c_include", "asm_include"):
        xpath = f'./include_paths/include_path[@type="{include_type}"]'
        parent = manifest_path.parent

        include_paths["None"].extend(
...
def _parse_sources(
    ...
    Schema:
        <component id="{component_id}" package_base_path="component">
          <source condition="{condition}" relative_path="./" type="src" device_cores="{device_core}...">
    ...
    Returns:
        dict of source files for the component, key is condition for this item, or "None" if there isn't condition.
    """
    sources = dict()
    ...
    for source in element.findall(xpath):
        ...
        for file in source.findall("./files"):
            ...
            if filename.suffix != ".ldt":
                ...
                sources[condition].append(source_absolute)
    return sources
...
def parse_headers(
    ...
    Returns:
        dict of header files for the component, key is condition for this item, or "None" if there isn't condition.
    """
    ...
    for source_type in ("src", "src_c", "src_cpp", "asm_include"):
        ...
        for key in parsed_sources:
            if key not in sources:
                sources[key] = list()
            sources[key].extend(parsed_sources[key])
    return sources
...
class Component:
    ...
    Properties:
        ...
        include_dirs: dict of include directory paths needed for this component, key is condition for this item, or "None" if there isn't condition.
        headers: dict of header paths exported by this component, key is condition for this item, or "None" if there isn't condition.
        sources: dict of source file paths built as part of this component, key is condition for this item, or "None" if there isn't condition.
        libs: dict of paths to prebuilt libraries required by this component, key is condition for this item, or "None" if there isn't condition.
    ...
    include_dirs: dict[str, list[pathlib.Path]] = field(default_factory=dict)
    headers: dict[str, list[pathlib.Path]] = field(default_factory=dict)
    sources: dict[str, list[pathlib.Path]] = field(default_factory=dict)
    libs: dict[str, list[pathlib.Path]] = field(default_factory=dict)
    ...
    def from_manifest(
        ...
        def relative_to_root(paths: dict[str, list[pathlib.Path]]) -> dict[str, list[pathlib.Path]]:
            for condition in paths:
                try:
                    paths[condition] = [path.resolve().relative_to(root_path) for path in paths[condition]]
                except ValueError:
                    paths[condition] = []
            return paths
    ...
    def remove_dependency(self, dep: "Component"):
        self.dependencies.discard(dep.id)

    def extract_common(self, other: "Component") -> "Component":
        """Create new component by combining two other"""

        merge_list = lambda a, b: sorted(set(a).union(b))

        self.defines = merge_list(self.defines, other.defines)
        for condition in self.include_dirs.keys() | other.include_dirs.keys():
            self.include_dirs[condition] = merge_list(self.include_dirs.get(condition, []), other.include_dirs.get(condition, []))
        for condition in self.headers.keys() | other.headers.keys():
            self.headers[condition] = merge_list(self.headers.get(condition, []), other.headers.get(condition, []))
        for condition in self.sources.keys() | other.sources.keys():
            self.sources[condition] = merge_list(self.sources.get(condition, []), other.sources.get(condition, []))
        for condition in self.libs.keys() | other.libs.keys():
            self.libs[condition] = merge_list(self.libs.get(condition, []), other.libs.get(condition, []))
        self.dependencies = self.dependencies.union(other.dependencies)

        self.dependencies.discard(self.id)
        self.dependencies.discard(other.id)
...
class Project:
    ...
    def cleanup_unknown_files(self):
        ...
        def cleanup_empty_dirs(path: pathlib.Path):
        ...
        for component in self.components.values():
            src_files.update(
                map(abspath, chain.from_iterable(component.sources.values())),
                map(abspath, chain.from_iterable(component.headers.values())),
                map(abspath, chain.from_iterable(component.libs.values())),
            )

        # pylint: disable=unexpected-keyword-arg
        for file in glob.iglob(
            f"{self.root}/**", recursive=True, include_hidden=True
        ):
