...
class Component:
    ...
    def extract_common(self, other: "Component") -> "Component":
        ...
        merge = lambda a, b: sorted(set(a).union(b))
        merged = Component(
            f"{self.id}.{other.id}",
            defines=merge(self.defines, other.defines),
            include_dirs=merge(self.include_dirs, other.include_dirs),
            headers=merge(self.headers, other.headers),
            sources=merge(self.sources, other.sources),
            libs=merge(self.libs, other.libs),
            private=True,
        )

        self.dependencies.add(merged.id)
        other.dependencies.add(merged.id)
