...
static bool GetMaxDownloadSize(Device* device,
                               const std::vector<std::string>& /* args */,
                               std::string* message) {
  *message = std::to_string(device->download_data().max_size());
  return true;
}
...
static void GetAllVars(Device* device,
  ...
  for (const auto& args : all_args) {
    ...
    auto arg_string = stringutils::Join(pw::span{args}, ":");
...
VariableProvider::VariableProvider() {
  // Register some basic fastboot variables (protocol version, max download size
  // and the "all" meta-variable)
  RegisterVariable(FB_VAR_VERSION, GetVersion, nullptr);
  RegisterVariable(FB_VAR_MAX_DOWNLOAD_SIZE, GetMaxDownloadSize, nullptr);
