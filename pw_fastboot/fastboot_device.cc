...
Device::Device(std::unique_ptr<Transport> transport,
    ...
    : kCommandMap({
      ...
      download_data_(
          std::make_unique<pw::Vector<char, PW_FASTBOOT_MAX_DOWNLOAD_SIZE>>()) {
}
...
bool Device::HandleData(bool read, pw::Vector<char>& data) {
...
void Device::ExecuteCommands() {
  ...
  for (;;) {
    ...
    const auto result = found_command->second(this, args);
    WriteStatus(result.result(), result.message());
  }
}
